...
beforeAll(async () => {
  ...
  await mongoose.connection.collection('useraccounts').deleteMany({ email: { $in: ['doneefav@test.com', 'frfav@test.com'] } })
  await mongoose.connection.collection('userprofiles').deleteMany({ profileName: { $in: ['Donee Fav Test Profile', 'FR Fav Test Profile'] } })
  ...
  const frProfile = await UserProfile.create({
    profileName: 'FR Fav Test Profile',
    description: 'For FR favourite testing',
...
describe('TC-23: View Favourite List', () => {
  it('TC23-1: should return a single favourite by valid ID', async () => {
    ...
    expect(res.body.success).toBe(true)
    expect(res.body.data._id).toBe(favouriteId)
  })
