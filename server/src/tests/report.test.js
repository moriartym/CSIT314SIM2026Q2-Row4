...
beforeAll(async () => {
  ...
  await mongoose.connection.collection('useraccounts').deleteMany({ email: { $in: ['pmreport@test.com', 'frreport@test.com'] } })
  await mongoose.connection.collection('userprofiles').deleteMany({ profileName: { $in: ['PM Report Test Profile', 'FR Report Test Profile'] } })
  ...
  const frProfile = await UserProfile.create({
    profileName: 'FR Report Test Profile',
    description: 'For FR report testing',
  ...
  const frUser = await UserAccount.create({
    username: 'frreportuser',
    email: 'frreport@test.com',
    password: hashedPassword,
